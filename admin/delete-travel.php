...
if (strlen($_SESSION['alogin']) == 0) {
	...
	if (isset($_POST['submit'])) {
		$sql = "DELETE from TblTravel where TravelId=:pid";
		$query = $dbh->prepare($sql);
		$query->bindParam(':pid', $pid, PDO::PARAM_STR);
		$query->execute();
		$msg = "Travel Deleted Successfully";
	}
...
?>
	...
	<head>
		<title>T&TMS | Admin Travel Deletion</title>
								...
								if ($query->rowCount() > 0) {
									foreach ($results as $result) {	?>
										...
										<form class="form-horizontal" name="travel" method="post" enctype="multipart/form-data">
											...
											<div class="form-group">
												...
												<div class="col-sm-8">
													<img src="travelimages/<?php echo htmlentities($result->TravelImage); ?>" width="200">
												</div>
											</div>
									<?php }
								} ?>
									...
									<div class="row">
										<div class="col-sm-8 col-sm-offset-2">
											<button type="submit" name="submit" class="btn-danger btn">Delete This Travel</button>
										</div>
									</div>
