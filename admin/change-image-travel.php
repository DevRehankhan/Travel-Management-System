...
if (strlen($_SESSION['alogin']) == 0) {
	...
	if (isset($_POST['submit'])) {
		...
		$pimage = $_FILES["travelimage"]["name"];
		move_uploaded_file($_FILES["travelimage"]["tmp_name"], "travelimages/" . $_FILES["travelimage"]["name"]);
		$sql = "update TblTravel set TravelImage=:pimage where TravelId=:imgid";
						...
						<div class="tab-content">
							<div class="tab-pane active" id="horizontal-form">
								<form class="form-horizontal" name="travel" method="post" enctype="multipart/form-data">
									<?php
									$imgid = intval($_GET['imgid']);
									$sql = "SELECT TravelImage from TblTravel where TravelId=:imgid";
									$query = $dbh->prepare($sql);
									$query->bindParam(':imgid', $imgid, PDO::PARAM_STR);
									$query->execute();
